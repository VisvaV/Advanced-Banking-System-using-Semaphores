...
void initialize_sample_data(vector<Account>& accounts) {
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        cout << "Created Account " << i << " ("
             << (type == AccountType::SAVINGS ? "Savings" :
                 type == AccountType::CHECKING ? "Checking" : "Business")
    ...
    switch (choice) {
        ...
        case 3: {
            ...
            if (file.is_open()) {
                ...
                string name, pin;
                while (file >> id >> type) {
                    file.ignore(); // Skip space before name
                    getline(file, name, '"'); // Read up to first quote (might be empty)
                    getline(file, name, '"'); // Read the name between quotes
    ...
    do {
        ...
        switch (choice) {
            ...
            case 8:
                ...
                cout << "Enter PIN: "; cin >> pin;
                if (account_id < accounts.size()) {
                    logs = Transaction::load_transaction_logs(); // Refresh logs before generating statement
